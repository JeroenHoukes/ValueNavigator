...
export function AIDataWithAuth() {
    ...
    return (
      <div className="space-y-4">
        ...
        <p className="text-slate-300 max-w-2xl">
          Sign in with your Entra ID to view and edit data. The database will show only rows you are allowed to see.
        </p>
        <button
          type="button"
          onClick={handleLogin}
    ...
    return (
      <div className="space-y-4">
        ...
        <p className="text-slate-400 text-sm">
          Signed in as <span className="font-mono">{accounts[0]?.username}</span>. Ensure this user exists in the Azure SQL database and has SELECT on table_ai.
        </p>
      </div>
    );
  }
      ...
      <header className="space-y-1">
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          <span className="text-sm text-slate-400">
            Signed in as <span className="font-mono">{accounts[0]?.username}</span>
          </span>
        </div>
        <p className="text-slate-300 max-w-2xl">
